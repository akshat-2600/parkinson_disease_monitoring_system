...
const PageDashboard = (() => {
      ...
      <div id="dashboardData" style="display:none">
        ...
        <div class="grid-4 section-gap">
          ...
          <div class="stat-card card-glow-violet">
            ...
            <div class="stat-sub"><span>Total motor</span><span class="stat-change neutral" id="updrsChange">—</span></div>
          </div>
          <div class="stat-card card-glow-green">
            <div class="stat-label" data-tooltip="Fusion model confidence">Fusion Confidence</div>
        ...
        <div class="card card-glow-cyan section-gap">
          ...
          <div class="chart-wrap" style="height:220px"><canvas id="progressionChart"></canvas></div>
        </div>

        <!-- Risk + Alerts -->
        <div class="grid-2">
          <div class="card">
            <div class="card-header"><span class="card-title">Risk Indicators</span><div class="card-icon" style="background:var(--red-dim)">⚠️</div></div>
            <div class="risk-list" id="riskList"></div>
          </div>
          <div class="card">
            <div class="card-header"><span class="card-title">Active Alerts</span><div class="card-icon" style="background:var(--amber-dim)">🔔</div></div>
  ...
  async function load() {
    ...
    let data;
    try {
      const res  = await API.getDashboard(App.patientId, Auth.getToken());
      data = res.data;
    } catch (e) {
      Helpers.showError('dashboardError', `API error: ${e.message}`);
  ...
  function _render(d) {
    ...
    if (sb) {
      ...
      sb.textContent = label; sb.className = `status-badge ${cls}`;
    }
    ...
    grid.innerHTML = Object.entries(modalities).slice(0, 6).map(([key, val], i) => {
      ...
      return `
        <div class="modality-item" data-tooltip="${labels[key]||key} confidence">
          ...
          <div class="modality-bar-wrap"><div class="modality-bar" style="width:${pct}%;background:${color}"></div></div>
        </div>`;
    }).join('');
  }
  ...
  function _renderProgressionChart(baseVal) {
    ...
    Charts.create('progression', ctx, {
      type: 'line',
      data: { labels, datasets: [{ label:'Severity', data:vals, borderColor:'#00d4ff', backgroundColor: Charts.gradientFill(ctx,'00d4ff',220), fill:true, tension:0.4, pointRadius:0, pointHoverRadius:5, borderWidth:2 }] },
      options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{display:false}, tooltip:Charts.TOOLTIP }, scales:{ x:{...Charts.SCALE.x, ticks:{maxTicksLimit:8}}, y:{...Charts.SCALE.y, min:Math.max(0,baseVal-20), max:Math.min(100,baseVal+20)} }, interaction:{mode:'nearest',axis:'x',intersect:false} },
    });
  }
  ...
  function _renderRisks(risks) {
    ...
    Helpers.setHTML('riskList', risks.length
      ? risks.map(r => `<div class="risk-item"><div class="risk-dot" style="background:${cm[r.level]||'var(--cyan)'}"></div><div class="risk-name">${r.name}</div><div class="risk-level risk-${r.level||'low'}">${(r.level||'low').toUpperCase()}</div></div>`).join('')
  ...
  function _renderAlerts(alerts) {
    Helpers.setHTML('alertList', alerts.length
      ? alerts.map(a => `<div class="alert-item alert-${a.type||'info'}"><div class="alert-header"><span class="alert-type">${a.type||'info'}</span><span class="alert-time">${a.time||''}</span></div><div class="alert-msg">${a.msg||a.message||''}</div></div>`).join('')
