...
const PatientDashboard = (() => {
  ...
  function _patchTopbar(user) {
    ...
    // Hide patient selector — patient can only see their own data
    ...
    root.innerHTML = `
      <div class="patient-welcome">
        ...
        <div class="welcome-text">
          ...
          <div style="font-size:11px;font-family:var(--font-mono);color:var(--cyan);margin-top:6px">
            Patient ID: ${uid || '—'}
          </div>
        </div>
      ...
      <div class="modal-backdrop" id="reportsModal">
        <div class="modal" style="max-width:600px">
          ...
          <div id="reportsModalContent" style="max-height:60vh;overflow-y:auto"></div>
        </div>
      </div>`;
  ...
  async function _loadData(uid) {
    ...
    try {
      // FIXED: No token argument passed — client.js reads token internally
      ...
      Helpers.setText('mySeverity',   d.severity ?? '—');
      ...
      if (sb) {
        const m = { stable:['🟢 Stable','status-stable'], warning:['🟡 Monitoring','status-warning'], critical:['🔴 Critical','status-critical'] };
        const [l, c] = m[d.status] || m.stable;
        sb.textContent = l; sb.className = `status-badge ${c}`;
      }
    ...
    } catch { Helpers.setText('myPredCount', '—'); }
        ...
        if (ctx) {
          Charts.create('myProgression', ctx, {
            ...
            options: {
              ...
              scales: {
                x: { ...Charts.SCALE.x, ticks: { maxTicksLimit: 6 } },
    ...
    try {
      ...
      Helpers.setHTML('myReportsList', reports.length
        ? reports.map(rp => `
            <div class="report-card section-gap">
              <div class="report-title">${rp.title || 'Report'}</div>
              <div class="report-meta"><span>📅 ${new Date(rp.created_at).toLocaleDateString()}</span></div>
              ${rp.content?.notes ? `<div class="report-body">${rp.content.notes}</div>` : ''}
            </div>`).join('')
      ...
      Helpers.setHTML('myReportsList', `<div class="empty-state"><div class="empty-text">Could not load reports</div></div>`);
    }
  }
  ...
  function showReports() {
    ...
    // FIXED: No token arg
    API.getPatientReports(uid)
      .then(r => {
        const reports = r.data?.reports || [];
        Helpers.setHTML('reportsModalContent', reports.length
          ? reports.map(rp => `<div class="report-card section-gap"><div class="report-title">${rp.title || 'Report'}</div><div class="report-meta">📅 ${new Date(rp.created_at).toLocaleDateString()}</div>${rp.content?.notes ? `<div class="report-body">${rp.content.notes}</div>` : ''}</div>`).join('')
