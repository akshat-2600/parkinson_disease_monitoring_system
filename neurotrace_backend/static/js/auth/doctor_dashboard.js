...
const DoctorDashboard = (() => {
  ...
  function _patchTopbar() {
    ...
    badge.className = 'role-badge doctor'; badge.textContent = '👨‍⚕️ Doctor'; right.prepend(badge);
  ...
  function _renderDoctorDashboard() {
    ...
    root.innerHTML = `
      <div class="doctor-header">
        <div class="doctor-info">
          <div class="doctor-avatar-wrap">
            <div class="doctor-avatar" id="docAvatar">${_ini(user)}</div>
      ...
      <div class="card card-glow-cyan section-gap">
        <div class="card-header"><span class="card-title">All Patients</span><span class="text-mono" style="font-size:11px;color:var(--text-3)" id="patientCount">Loading…</span></div>
        <div class="patient-table-wrap">
          <table class="patient-table"><thead><tr><th>Patient</th><th>Age</th><th>Gender</th><th>Diagnosis</th><th>Severity</th><th>Status</th><th>Last Prediction</th><th></th></tr></thead>
          <tbody id="patientTableBody"><tr><td colspan="8" style="text-align:center;padding:40px"><div class="spinner" style="margin:auto"></div></td></tr></tbody></table>
        </div>
      </div>
      <div class="grid-4">
    ...
    document.getElementById('patientSearch')?.addEventListener('input', e => {
      ...
      _renderTable(_patients.filter(p => (p.name||'').toLowerCase().includes(q) || (p.patient_uid||'').toLowerCase().includes(q)));
    });
  }

  async function _loadAll() {
    try {
      const res = await API.listPatients(Auth.getToken(), 1, 200);
  ...
  function _renderTable(patients) {
    ...
    const tbody = document.getElementById('patientTableBody'); if(!tbody) return;
    if (!patients.length) { tbody.innerHTML = `<tr><td colspan="8"><div class="empty-state"><div class="empty-icon">👤</div><div class="empty-text">No patients found</div></div></td></tr>`; return; }
    ...
    tbody.innerHTML = patients.map(p => {
      ...
      return `<tr onclick="DoctorDashboard.openPatient('${p.patient_uid}')">
        <td><div class="pt-name-cell"><div class="pt-mini-avatar" style="background:${ac}">${ini||'?'}</div><div><div style="font-weight:500">${nm}</div><div class="pt-uid">${p.patient_uid}</div></div></div></td>
        <td>${p.age||'—'}</td><td>${p.gender||'—'}</td>
        <td style="font-size:12px;color:var(--text-2)">Parkinson's Disease</td>
        <td>${sev!=null?`<span class="severity-pill ${sc}">${sev.toFixed(0)}</span>`:`<span style="color:var(--text-3);font-size:12px">No data</span>`}</td>
        <td style="font-size:12px">${st}</td>
        <td style="font-size:11px;font-family:var(--font-mono);color:var(--text-3)">${p.latest_prediction?.created_at?new Date(p.latest_prediction.created_at).toLocaleDateString():'—'}</td>
        <td><button class="btn btn-ghost" style="font-size:11px;padding:5px 10px" onclick="event.stopPropagation();DoctorDashboard.openPatient('${p.patient_uid}')">View →</button></td>
      </tr>`;
    }).join('');
  }
  ...
  function _updateStats() {
    ...
    const avg     = withSev.length ? Math.round(withSev.reduce((s,p)=>s+p.latest_severity,0)/withSev.length) : 0;
  ...
  function filterSeverity(level, btn) {
    document.querySelectorAll('.table-search-bar .btn').forEach(b=>b.classList.remove('btn-primary'));
    btn.classList.add('btn-primary');
    const filtered = level==='all'?_patients:_patients.filter(p=>{const s=p.latest_severity||0;if(level==='critical')return s>=70;if(level==='moderate')return s>=40&&s<70;if(level==='stable')return s<40;return true;});
  ...
  function openPatient(uid) {
    App.setPatient(uid);
    Router.navigate('dashboard');
    setTimeout(() => PageDashboard.load(), 100);
  }
