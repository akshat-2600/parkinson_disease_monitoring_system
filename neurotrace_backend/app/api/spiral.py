"""
app/api/spiral.py
Spiral drawing prediction endpoint.

POST /spiral/predict — upload spiral image → CNN prediction
"""
import time
...
def predict():
    """
    POST /spiral/predict
    Form-data: spiral_image (file), patient_id (optional)
    """
    ...
    if patient_id:
        _persist(patient_id, "spiral", result)

    return prediction_response("spiral", result, patient_id=patient_id, processing_ms=elapsed)
