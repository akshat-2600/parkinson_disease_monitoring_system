/* ============================================================
   js/pages/history.js — Historical tracking page
   ============================================================ */
...
const PageHistory = (() => {
      ...
      <div id="historyData" style="display:none">
        ...
        <div class="card card-glow-cyan section-gap">
          <div class="card-header">
            ...
            <span class="text-muted" style="font-size:11px;font-family:var(--font-mono)">12-month view</span>
          </div>
        ...
        <!-- UPDRS + Multi-modality -->
        <div class="grid-2 section-gap">
          <div class="card card-glow-violet">
            <div class="card-header">
              <span class="card-title">UPDRS Scores</span>
            </div>
          ...
          <div class="card card-glow-green">
            <div class="card-header">
              <span class="card-title">Multi-Modality Trends</span>
            </div>
            <div class="chart-wrap" style="height:200px">
  ...
  async function load() {
    ...
    let data;
    try {
      data = await API.getHistory(App.patientId);
    } catch (e) {
      Helpers.showError('historyError', `API unreachable: ${e.message}`);
  ...
  function _render(d) {
    ...
    // Severity chart
    _lineChart('historySeverity', 'historySeverityChart', labels, [{
      label: 'Severity',
      data: d.severity || [],
      borderColor: '#00d4ff',
      fill: true,
    ...
    // UPDRS chart
    _lineChart('historyUpdrs', 'historyUpdrsChart', labels, [{
      label: 'UPDRS',
      data: d.updrs || [],
      borderColor: '#7b5cff',
    ...
    // Multi-modality
    _lineChart('historyModality', 'historyModalityChart', labels, [
    ...
    // Intervention timeline
    _renderTimeline(d.interventions || []);
  }

  function _lineChart(key, canvasId, labels, datasets, opts = {}) {
    const ctx = document.getElementById(canvasId)?.getContext('2d');
    if (!ctx) return;

    // Apply gradient fill to first dataset if fill:true
    datasets.forEach(ds => {
      if (ds.fill && ds.borderColor && !ds.backgroundColor) {
  ...
  function _renderTimeline(interventions) {
